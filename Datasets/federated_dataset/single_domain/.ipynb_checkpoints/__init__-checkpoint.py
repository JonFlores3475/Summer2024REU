...
def get_all_models():
    return [model.strip('.py') for model in os.listdir('Datasets/federated_dataset/single_domain')
            if not model.find('_') > -1 and 'py' in model]


single_domain_dataset_name = {}
for model in get_all_models():
    mod = importlib.import_module('Datasets.federated_dataset.single_domain.' + model.strip('.py'))
