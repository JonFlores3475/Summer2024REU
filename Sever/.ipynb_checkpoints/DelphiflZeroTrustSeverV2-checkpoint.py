...
class DelphiflZeroTrustSeverV2(SeverMethod):
    NAME = 'DelphiflZeroTrustSeverV2'

    def __init__(self, args, cfg):
        super(DelphiflZeroTrustSeverV2, self).__init__(args, cfg)
        
        nets_list1 = get_private_backbones(cfg)
        ...
        for name, param in copy.deepcopy(nets_list1[0]).cpu().state_dict().items():
            param = nets_list1[0].state_dict()[name].view(-1)
    ...
    def sever_update(self, **kwargs):
        ...
        with torch.no_grad():
            update_dict = {
                online_clients_list[i]:
                    parameters_to_vector(new[i].parameters()) - parameters_to_vector(global_net.parameters())
                for i in range(len(new))
            }
        
        lr_vector = self.compute_robustLR(update_dict)
        aggregated_updates = self.agg_avg(update_dict, self.current_weights, online_clients_list)
        ...
        for info in nets_list:
            info.load_state_dict(global_net.state_dict(), strict=False)
